...
Возможности:
 - [x] База персонажей и предметов актуальна для патча `3.0+3.1`, обновляется с выходом обновлений в игре
...
Каждый баннер представляет собой `JSON` файл, в котором есть следующие секции:
...
Внутри секции **wishes** находятся секции **5**, **4** и **3**, которые обозначают количество звезд у предмета. Внутри каждой из них указываются предметы, доступные для этого баннера:
...
Перед всеми командами используется префикс из **wish_command_prefix**
 ...
 - **gbot_status** - выводит информацию о молитвах в чат
 - **gbot_stats** - выводит информацию о боте

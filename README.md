...
### Регистритуем приложение (необязательно, но рекомендуется)

Заходим в свой основной аккаунт Twitch, с которого будет вестись трансляция, переходим в [панель разработчика](https://dev.twitch.tv/console/apps), выбираем регистрацию приложения. 
...
Проходим капчу и подтверждаем создание приложения. Переходим обратно к списку всех приложений и выбираем то, которое только что создали, жмем *Управление*
![данные приложения](https://media.discordapp.net/attachments/706496930414592080/984119131135672370/unknown.png)

Жмем на кнопку *Новый секретный код*, появится новое поле. Открываем в новой вкладке (текущую не закрываем) [генератор Twitch токенов](https://twitchtokengenerator.com/), жмем на *Uhhhh what? Just take me to the site*
...
Заполняем поля данными из предыдущей вкладки:
...
Если разные, то для чатбота отмечаем только:
 - **chat:read** - чтобы читать сообщения из чата
 - **chat:edit** - чтобы писать в чат

 Для бота баллов канала (только для аккаунта, *с которого будет вестись трансляция*):
  - **channel:read:redemptions** - чтобы видеть использование балов канала
...
Прокручиваем до конца списка и жмем кнопку *Generate Token!* Обязательно проверяем, что вверху страницы указано название нашего приложения (если создавали) и имя нужного нам аккаунта.
...
Копируем токен и вставляем в файл конфигурации **config.json**:
 ...
 - если аккаунты для чатбота и баллов канала разные:
   ...
   - повторяем шаги из *Создаем токен*
...
## Описание параметров из config.json:
 ...
 - **event_bot** - секция для настройки баллов канала
   ...
   - **work_channel_id** - ID канала на котором будут использоваться награды (как его узнать написано в конце этой страницы)
